...
private:
    ...
    //returns a unique_ptr to a _task_container that wraps around a Callable
    //  for details on _task_container_base and _task_container, see above. This
    //  exists so that _Func may be inferred from f.
    //
    //  Note: currently, compiler support is bad for deduction guides. However,
    //        in a perfect world, `allocate_task_container` can be completely
    //        replaced by adding a deduction guide to the `_task_container` ctor:
    //        _task_container(F&&) -> _task_container<std::decay<F>>
...
auto thread_pool::execute(F &&function, Args &&... args) {
    //auto err = _task_container([](int x) {});
    ...
    // packaged_task
    //  type is not CopyConstructible, the function is not CopyConstructible
    //  either - hence the need for a _task_container to wrap around it.
    _tasks.emplace(allocate_task_container(
